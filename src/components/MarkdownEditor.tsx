...
export const MarkdownEditor = forwardRef<IMarkdownEditorRef, IMarkdownEditorProps>(
  (
    {
      value,
      onChange,
    ...
    const wrappedOnImageSourceSelect = useCallback(
      async (file: File) => {
        const url = onImageSourceSelect
          ? await onImageSourceSelect(file)
    ...
    useImperativeHandle(
      ...
      () => ({
        getMarkdown: () => {
          if (!editor) return '';
          return JsonToMarkdownConverter.convertToMarkdown(editor.getJSON());
        },
        getPendingFiles: () => {
          return new Map(pendingImagesRef.current);
        },
        finalizeImages: async (uploadFn) => {
          if (!editor) return '';

          const currentMarkdown = JsonToMarkdownConverter.convertToMarkdown(editor.getJSON());
          const pendingFiles = pendingImagesRef.current;

          if (pendingFiles.size === 0) return currentMarkdown;

          // Find all unique blob: URLs in markdown
          // Matching pattern: ![...](blob:...)
          const blobUrlRegex = /!\[.*?\]\((blob:[^)]+)\)/g;
          const matches = currentMarkdown.matchAll(blobUrlRegex);
          const uniqueBlobUrls = new Set<string>();
          for (const match of matches) {
            uniqueBlobUrls.add(match[1]);
          }

          if (uniqueBlobUrls.size === 0) return currentMarkdown;

          let finalizedMarkdown = currentMarkdown;
          ...
          for (const blobUrl of uniqueBlobUrls) {
            ...
            if (file) {
              try {
                ...
                urlReplacements[blobUrl] = serverUrl;
                // Clean up registry if successful
                pendingFiles.delete(blobUrl);
                URL.revokeObjectURL(blobUrl);
              } catch (error) {
                logger.error(`Failed to upload image ${blobUrl}:`, error);
              }
            }
          }

          // Replace URLs in markdown
          for (const [blobUrl, serverUrl] of Object.entries(urlReplacements)) {
            // Use global replace with escaping special characters
            const escapedBlobUrl = blobUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            finalizedMarkdown = finalizedMarkdown.replace(
              new RegExp(escapedBlobUrl, 'g'),
              serverUrl,
            );
          }

          // Optionally update editor content with finalized markdown
          if (Object.keys(urlReplacements).length > 0) {
            setIsUpdating(true);
            try {
              const json = await MarkdownTipTapConverter.markdownToTipTapJson(finalizedMarkdown, {
                publicImagePathPrefix,
              });
              editor.commands.setContent(json);
            } catch (error) {
              logger.error('Failed to update editor with finalized images:', error);
            } finally {
              setIsUpdating(false);
            }
          }

          return finalizedMarkdown;
        },
      }),
      [editor, publicImagePathPrefix, setIsUpdating],
    );
        ...
        try {
          ...
          if (codeBlocks.length > 0) {
            codeBlocks.forEach((block, idx) => {
              ...
              logger.info(
                `CodeBlock ${idx}: ${block.attrs?.language || 'no-lang'}, ${lines} lines`,
              );
            });
          }
              ...
              <div className="bg-background border border-border rounded-lg shadow-lg p-3 flex items-center space-x-2 min-w-48">
                ...
                <div className="text-xs">
                  <div className="font-medium text-foreground">
                    {t(I18N_KEYS.convertingMarkdown)}
                  </div>
            ...
            <div
                ...
                if (imageFiles.length > 0 && editor) {
                  ...
                  for (const file of imageFiles) {
                    try {
                      ...
                      logger.error('Container drop upload failed', err);
                    }
                  }
                }
              }}
              onMouseDown={(e) => {
                ...
                if (editor && editable) {
                  ...
                  // If clicking outside the content editable area
                  if (proseMirrorElement && !proseMirrorElement.contains(target)) {
                    // Always focus end when clicking the background
                    const { state } = editor;
