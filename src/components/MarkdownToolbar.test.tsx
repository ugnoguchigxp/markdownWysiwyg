...
describe('MarkdownToolbar', () => {
  ...
  it('opens link modal and inserts markdown link', () => {
    ...
    const linkBtn = container.querySelector(
      `[data-tooltip="${I18N_KEYS.insertLink}"]`,
    ) as HTMLElement;
  ...
  it('shows download menu and triggers download handler', () => {
    ...
    const markdownFileBtn = screen.getByRole('button', {
      name: new RegExp(I18N_KEYS.markdownFile),
    });
  ...
  it('falls back to markdown table insertion when editor is not provided', () => {
    ...
    const { container } = render(<MarkdownToolbar onInsertMarkdown={onInsertMarkdown} />);

    const tableBtn = container.querySelector(
      `[data-tooltip="${I18N_KEYS.insertTable}"]`,
    ) as HTMLElement;
