...
describe('security', () => {
  ...
  describe('normalizeImageSrcOrNull', () => {
    ...
    it('should reject javascript: URLs', () => {
      expect(normalizeImageSrcOrNull('javascript:alert(1)', { publicPathPrefix: '' })).toBe(null);
    });

    it('should reject data: URLs', () => {
      expect(
        normalizeImageSrcOrNull('data:image/png;base64,ABC123', { publicPathPrefix: '' }),
      ).toBe(null);
    });
  ...
  describe('sanitizeSvg', () => {
    ...
    it('should remove event handler attributes', () => {
      const svg =
        '<svg><rect onclick="alert(1)" onerror="alert(2)" width="100" height="100"/></svg>';
      const result = sanitizeSvg(svg);
    ...
    it('should remove data:text/html href attributes', () => {
      const svg =
        '<svg><a href="data:text/html,<script>alert(1)</script>"><text>Click</text></a></svg>';
      const result = sanitizeSvg(svg);
