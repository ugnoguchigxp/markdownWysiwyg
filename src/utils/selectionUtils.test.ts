...
describe('SelectionUtils', () => {
  describe('getSelectionMarkdownSyntax', () => {
    ...
    it('should handle strikethrough mark at cursor position', () => {
      const marks = [{ type: { name: 'strike' } }];
      const editor = createMockEditor(
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Strikethrough');
      expect(info?.markdownSyntax).toBe('~~取り消し線~~');
    });

    it('should handle inline code mark at cursor position', () => {
      const marks = [{ type: { name: 'code' } }];
      const editor = createMockEditor(
        { from: 0, to: 0, empty: true },
        '',
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Inline Code');
      expect(info?.markdownSyntax).toBe('`インラインコード`');
    });

    it('should handle link mark at cursor position', () => {
      const marks = [{ type: { name: 'link' }, attrs: { href: 'http://example.com' } }];
      const editor = createMockEditor(
        { from: 0, to: 0, empty: true },
        '',
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Link');
      expect(info?.markdownSyntax).toBe('[リンク](http://example.com)');
    });

    it('should handle italic mark at cursor position', () => {
      const marks = [{ type: { name: 'italic' } }];
      const editor = createMockEditor(
        { from: 0, to: 0, empty: true },
        '',
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Italic');
      expect(info?.markdownSyntax).toBe('*斜体*');
    });

    it('should handle bold mark at cursor position', () => {
      const marks = [{ type: { name: 'bold' } }];
      const editor = createMockEditor(
        { from: 0, to: 0, empty: true },
        '',
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Bold');
      expect(info?.markdownSyntax).toBe('**太字**');
    });

    it('should handle strong mark (alternative to bold)', () => {
      const marks = [{ type: { name: 'strong' } }];
      const editor = createMockEditor(
        { from: 0, to: 5, empty: false },
        'Text',
        marks,
    ...
    it('should handle em mark (alternative to italic)', () => {
      const marks = [{ type: { name: 'em' } }];
      const editor = createMockEditor(
        { from: 0, to: 5, empty: false },
        'Text',
        marks,
    ...
    it('should handle codeBlock at cursor position', () => {
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Code Block');
      expect(info?.markdownSyntax).toContain('```python');
      expect(info?.markdownSyntax).toContain('(コードブロック)');
    });

    it('should handle blockquote at cursor position', () => {
      const editor = createMockEditor({ from: 0, to: 0, empty: true }, '', [], {
        type: { name: 'blockquote' },
        attrs: {},
      }) as unknown as Editor;
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Blockquote');
      expect(info?.markdownSyntax).toBe('> (引用)');
    });

    it('should handle listItem at cursor position', () => {
      const editor = createMockEditor({ from: 0, to: 0, empty: true }, '', [], {
        type: { name: 'listItem' },
        attrs: {},
      }) as unknown as Editor;
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('List Item');
      expect(info?.markdownSyntax).toBe('- (リスト項目)');
    });

    it('should handle heading with default level (1) when level is not number', () => {
    ...
    it('should handle codeBlock with default language (empty)', () => {
      const editor = createMockEditor({ from: 0, to: 5, empty: false }, 'code', [], {
        type: { name: 'codeBlock' },
        attrs: { language: null },
      }) as unknown as Editor;
      ...
      const info = SelectionUtils.getSelectionMarkdownSyntax(editor);
      expect(info?.marks).toContain('Code Block');
      expect(info?.markdownSyntax).toBe('```\ncode\n```');
    });
